...
public:
	...
	inline bool InputState::pressed() const { return m_state == State::Pressed; }
	inline bool InputState::down() const { return m_state == State::Pressed || m_state == State::Down; }
	inline bool InputState::released() const { return m_state == State::Released; }
	inline bool InputState::up() const { return m_state == State::Released || m_state == State::Up; }
	inline bool operator== (const State& rhs) const { return m_state == rhs; }
	inline bool operator!= (const State& rhs) const { return m_state != rhs; }
};
...
public:
	...
	inline const InputState* Input::getState(sf::Keyboard::Key key) { return &m_keyStates[key]; }
	inline const InputState* Input::getState(sf::Mouse::Button button) { return &m_buttonStates[button]; }
	inline bool pressed(sf::Keyboard::Key key) { return m_keyStates[key].pressed(); }
	inline bool down(sf::Keyboard::Key key) { return m_keyStates[key].down(); }
	inline bool released(sf::Keyboard::Key key) { return m_keyStates[key].released(); }
	inline bool up(sf::Keyboard::Key key) { return m_keyStates[key].up(); }
	inline bool pressed(sf::Mouse::Button button) { return m_buttonStates[button].pressed(); }
